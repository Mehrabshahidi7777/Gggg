...
@section('meta_description', $type==='product'
    ? 'خرید مصالح و محصولات ساختمانی از تأمین‌کنندگان سراسر کشور؛ آجر، سیمان، آهن‌آلات، در و پنجره، تاسیسات و ابزار. تماس مستقیم با فروشنده.'
    : 'خدمات ساختمانی از متخصصان سراسر کشور؛ ساخت و ساز، بازسازی، دکوراسیون، تاسیسات مکانیکی و برقی، نما و عایق‌کاری. تماس مستقیم با ارائه‌دهنده.'
)

{{--
    canonical صفحه‌های فهرست همیشه آدرس پایه است، بدون پارامترهای
    فیلتر و صفحه‌بندی.

    بدون این، هر ترکیب از ?page و ?sort و ?province و ?city یک صفحه‌ی
    مجزا با محتوای تقریباً یکسان می‌شد. گوگل آن را «محتوای تکراری»
    می‌بیند و اعتبار صفحه بین ده‌ها آدرس پخش می‌شود.
--}}
@section('canonical', route($type==='product' ? 'products' : 'services'))
