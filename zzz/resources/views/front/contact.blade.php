...
--}}<div class="hp-field" aria-hidden="true"><label for="website">وب‌سایت</label><input id="website" type="text" name="website" value="" tabindex="-1" autocomplete="off"></div><input type="hidden" name="opened_at" value="{{ encrypt(time()) }}"><input name="name" value="{{ old('name') }}" class="w-full rounded-lg border" placeholder="نام و نام خانوادگی" required maxlength="255"><input name="phone" value="{{ old('phone') }}" maxlength="15" inputmode="numeric" pattern="[0-9]*" data-digits-only class="w-full rounded-lg border" placeholder="شماره تماس" required><input name="email" type="email" value="{{ old('email') }}" class="w-full rounded-lg border" placeholder="ایمیل (اختیاری)"><input name="subject" value="{{ old('subject') }}" class="w-full rounded-lg border" placeholder="موضوع"><textarea name="message" rows="6" class="w-full rounded-lg border" placeholder="پیام شما" required maxlength="5000">{{ old('message') }}</textarea><button class="btn-primary" data-gate-submit>ارسال پیام</button></form></div></div>@endsection
