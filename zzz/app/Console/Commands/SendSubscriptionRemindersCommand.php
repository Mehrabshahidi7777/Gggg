        ...
        $query->chunkById(100, function ($subscriptions) use ($sms, $dry, $column, $daysLeft, $now, &$count) {
            ...
            foreach ($subscriptions as $subscription) {
                ...
                */
                ...
                |
                | ⚠️ و روزِ تقویمی شمرده می‌شود، نه کسرِ روز.
                |
                | کرون ساعت ۱۰ اجرا می‌شود ولی اشتراک هر ساعتی ممکن
                | است تمام شود. اشتراکی که ۱۹ آبان ساعت ۱۴:۰۹ تمام
                | می‌شود، ۱۳ آبان ساعت ۱۰ دقیقاً ۶٫۱۷ روز مانده دارد.
                |
                | با ceil می‌شد ۷، و کاربر ۱۳ + ۷ = ۲۰ حساب می‌کرد -
                | یعنی یک روز دیرتر از انقضای واقعی. همان یک روز
                | می‌توانست به قیمت تعلیق‌شدنِ آگهی‌اش تمام شود.
                |
                | با شمارشِ روزِ تقویمی می‌شود ۶، و ۱۳ + ۶ = ۱۹: همان
                | روزی که واقعاً تمام می‌شود. کاربر هم به ساعت فکر
                | نمی‌کند، به روز فکر می‌کند.
                */
                $remaining = (int) $now->copy()->startOfDay()
                    ->diffInDays($subscription->ends_at->copy()->startOfDay());

                $state = match (true) {
                    $daysLeft === 0 => 'تمام شد؛ آگهی‌ها تعلیق شدند',
                    $remaining <= 0 => 'امروز تمام می‌شود',
