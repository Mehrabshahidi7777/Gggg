...
class OrderController extends Controller
{
    /*
    |--------------------------------------------------------------------------
    | تاریخچه‌ی سفارش‌های آنلاین
    |--------------------------------------------------------------------------
    |
    | خرید آنلاین خاموش است (marketplace.online_checkout) و تماس مستقیم
    | انجام می‌شود، پس این صفحه برای تقریباً همه یک جدول خالی بود.
    |
    | از منو برداشته شد و جایش را «فعالیت‌های من» گرفت، ولی آدرسش باز
    | ماند - یعنی هرکسی که /my-orders را مستقیم می‌زد باز هم همان
    | صفحه‌ی خالی را می‌دید.
    |
    | حالا فقط برای کسی وجود دارد که واقعاً سفارشی دارد. برای بقیه
    | ۴۰۴ است، چون برایشان واقعاً چیزی آنجا نیست.
    |
    | ⚠️ سفارش‌های قدیمی پاک نمی‌شوند و صاحبشان همچنان می‌بیندشان.
    | اگر روزی خرید آنلاین روشن شود، اولین سفارش این صفحه را خودبه‌خود
    | برمی‌گرداند.
    */
    public function index()
    {
        ...
        abort_if($user->orders()->doesntExist(), 404);
