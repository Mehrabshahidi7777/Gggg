...
class AmootSmsService
{
    ...
    */
    ...
    |
    | دو متغیر: نوع اشتراک، و وضعیت.
    |
    | ⚠️ متغیر دوم عمداً یک *عبارت* است، نه تعداد روز.
    |
    ...
    |
    | ⚠️ نامِ کاربر عمداً در پیامک نیست.
    |
    | سقف یک صفحه‌ی پیامک فارسی ۷۰ کاراکتر است و از آن به بعد هزینه
    | دو برابر می‌شود. نامِ کاربری در این سایت هم انگلیسی است و تا ۵۰
    | حرف می‌تواند باشد - یعنی یک نام بلند، پیام را به سه صفحه
    | می‌برد. بدون نام، بدترین حالت ۶۵ کاراکتر است: همیشه یک صفحه.
    |
    | پیامک هم به شماره‌ی خودِ کاربر می‌رود و درباره‌ی اشتراک خودش
    | است، پس نام چیز زیادی اضافه نمی‌کرد.
    |
    */
